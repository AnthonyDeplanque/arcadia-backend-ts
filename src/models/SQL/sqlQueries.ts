...
export const getOneQuery = (table: string, id: string): Promise<any> => {
  return db.query(
    `SELECT * FROM ${table} WHERE ${table.toLocaleLowerCase()}_id = ? `,
    [id]
  );
};
...
export const updateQuery = (
  ...
  return db.query(
    `UPDATE ${table} SET ? WHERE ${table.toLocaleLowerCase()}_id = ?`,
    [values, id]
  );
};
...
export const deleteQuery = (table: string, id: string): Promise<any> => {
  return db.query(
    `DELETE FROM ${table} WHERE ${table.toLocaleLowerCase()}_id = ?`,
    [id]
  );
};
...
export const getOneUserByItsUsernameQuery = (
  username: string
): Promise<any> => {
